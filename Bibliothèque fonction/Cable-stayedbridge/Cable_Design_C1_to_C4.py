...
N_cable_sections = 30     # Number of cable sections
s_cables = 16500    # Horizontal distance between deck anchor points 
Edge_cables =['Cables-1','Cables-12','Cables-13','Cables-30'] # Edge cables
s_cables_e = 18625  # Edge cables deck tributary length 
...
ret = SapModel.PropFrame.SetSDSection('Deck', 'Concrete')
...
Deck_SD = SapModel.PropFrame.GetSDSection('Deck')
# Get Shape name
Deck_SD_Shape_Name = Deck_SD[2][0]
# Get polygon parameters: x,y coordinates
Deck_SD_Parameters = SapModel.PropFrame.SDShape.GetPolygon('Deck',Deck_SD_Shape_Name)
...
ret = SapModel.PropFrame.SDShape.SetPolygon('Deck',Deck_SD_Shape_Name, Deck_SD_Parameters[0] , Deck_SD_Parameters[1], Deck_SD_Parameters[2], Deck_SD_x, 
...
""" 
# Initialize top coordinates of each cables
z_Cable_Top = []
for i in range(Cable_names[0]):
    Cable_Points = SapModel.CableObj.GetPoints(Cable_names[1][i])
...
for l in range(len(Load_Cases)):
    ...
    # Get deck area
    Deck_area = SapModel.PropFrame.GetSectProps('Deck')[0]
    # Deck self-weight (N/mm)
    Deck_SW = Deck_area*w_c
    # Permanent loads
    Deck_per = Deck_SW + q_ballast
    ...
    """ 
    Iteration_Parameters = [Deck_v_allow_v,max_iter,0]
    if l == 0:
