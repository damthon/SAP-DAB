...
Make_Symm = 0        # Use equal areas for the symmetrical cables
...
for l in range(len(Load_Cases)):
    ...
    # Assign horizontal restraint to pylon connection with deck  
    for z in range(len(Pylon_Deck_Joints)):
        SapModel.PointObj.SetRestraint(Pylon_Deck_Joints[z], Pylon_Restraint)
    
    # Assign vertical temporary vertical restraint to backspan cables 
    Deck_BS_Restraint = [False, False, True, False, False, False]  # Ux, Uy, Uz, Rx, Ry, Rz
    for t in range(len(Cables_Deck_Temporary_Restraint)):
        SapModel.PointObj.SetRestraint(Cables_Deck_Temporary_Restraint[t], Deck_BS_Restraint)
    #########################################################################################################################
