...
def offsetPointsMoveSAP2000(SapModel,group_points,offset_type,offset_value):
    """
    This function moves a group points by specifying an offset value.
    Input:
    ...
    if offset_type[0] == 1: # Reference points
        ...
        if len(points_vector) != len(points):
            print('Number of vectors do not correspond to points to be moved. Operation aborted!')
            op = 0 
        else:
            # Point coordinates
            x_p, y_p, z_p = [], [], []
    ...
    if  op == 1:  
        for i in range(len(points_sorted)):
            ...
            v_move = unitVector(offset_v[i])
            x_move, y_move, z_move = v_move[0]*offset_value, v_move[1]*offset_value, v_move[2]*offset_value  # x, y, z displaced coordinates
            SapModel.EditGeneral.Move(x_move,y_move,z_move)
            print('Point ' + str(i) + ' out of ' + str(len(points_sorted)) + ' moved: ' + str(points_sorted[i]))
    return None
