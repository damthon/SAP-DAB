...
def sectionDim(SapModel,sec):
    ...
    x_coord_w = web[0]
    y_coord_w = web[1]
    # Additional elements/angles
    index_e = [] # Indices of additional elements
    add_e = [] # Additional elements
    x_coord_add_e = [] # x-coordinates of additional elements
    y_coord_add_e = [] # x-coordinates of additional elements
    for e in range(len(polygons_sec)):
        if polygons_sec[e] != 'Top_Plates' and polygons_sec[e] != 'Bottom_Plates' and polygons_sec[e] != 'Âme' and polygons_sec[e] != 'L70.70.9_L' and polygons_sec[e] != 'L70.70.9_R':
            index_e.append(e)
            add_e.append(polygons_sec[e])
    for e in index_e:
        add_e_temp = SapModel.PropFrame.SDShape.GetPolygon(sec,SapModel.PropFrame.GetSDSection(sec)[2][e])[3:5]
        add_e.append(add_e_temp)
        x_coord_add_e.append(add_e_temp[0])
        y_coord_add_e.append(add_e_temp[1])
    ...
    offset_y = -min(y_coord_bf)
    y_coord_add_e_shifted = [[] for _ in range(len(y_coord_add_e))]
    for i in range(len(y_coord_add_e)):
        y_coord_add_e_shifted[i] = [[] for j in range(len(y_coord_add_e[i]))]
        for j in range(len(y_coord_add_e[i])):
            y_temp = list(y_coord_add_e[i])
            y_coord_add_e_shifted[i][j] = y_temp[j] + offset_y
    
    # Get additional elements
    additional_elements = [[] for _ in range(len(x_coord_add_e))]
    for i in range(len(x_coord_add_e)):
        for j in range(len(x_coord_add_e[i])):
            additional_elements[i].append((x_coord_add_e[i][j],y_coord_add_e_shifted[i][j]))

    
    return h,b_tf,t_tf,t_w,b_bf,t_bf,additional_elements