...
def plotNormalStressProfileIsectionMB(P,Mxx,Myy,I_sec,fy,stress_profile,GROUP,scale_font,figure_dimensions,figure_save):
    """
    ...
    for i in range(len(polygons_tf)):
        Pol = Polygon(polygons_tf[i])
        x_temp, y_temp = Pol.exterior.xy
        x_tfi[i] = x_temp
        y_tfi[i] = y_temp
    ...
    ax1.fill(x_bf,y_bf,c=(157/255,188/255,212/255),alpha=.5)
    # Angles
    ...
    if discontinuity == 1:
        ...
        ax1.fill(x_bf,[0,0,y_profile_2[0],y_profile_2[0],0],c=(1,1,0),alpha=.5)
    # Centerline
    ...
    if y_wt[2] != y_wb[0]: # Discontinous web
        ...
        ax1.fill(x_wt,y_wt,c=(157/255,188/255,212/255),alpha=.5)
        # Web bottom
        ax1.plot(x_wb, y_wb, color=(0, 0, 0), linewidth=1.0)
        ax1.fill(x_wb,y_wb,c=(157/255,188/255,212/255),alpha=.5)
    else:
        # Web 
        ax1.plot(x_w, y_w, color=(0, 0, 0), linewidth=1.0)
        ax1.fill(x_w,y_w,c=(157/255,188/255,212/255),alpha=.5)
